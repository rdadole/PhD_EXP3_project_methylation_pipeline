...
# --- Command ---
echo "--- Starting Pileup for CHH context ---"
if [ "$barcodes" == "NA" ]; then
    ...
    if [ -f "$input_sorted_bam" ]; then
        modkit pileup -t $SLURM_CPUS_PER_TASK "$input_sorted_bam" "$output_bed_CHH" --ref "$reference" --motif CHH 0
    else
...
echo "--- Pileup for CHH context complete. ---"
